...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games AS games1 ON teams.team_id = games1.winner_id FULL JOIN games AS games2 ON team_id = games2.opponent_id WHERE (games1.year = 2014 OR games2.year = 2014) AND (games1.round = 'Eighth-Final' OR games2.round = 'Eighth-Final')")"
